from insights.client.auto_config import APIConfig, DeploymentType, ProxyConfig
...
@pytest.mark.parametrize(
    "rhel,config_legacy_upload,expected_url",
    [
        (9, True, API_CONFIG_LEGACY.url),
        (9, False, API_CONFIG_PRODUCTION.url),
        (10, False, API_CONFIG_PRODUCTION.url),
    ],
    ids=["el9,legacy=True", "el9,legacy=False", "el10"],
)
@mock.patch("insights.client.auto_config.rhsmCertificate.existsAndValid", return_value=True)
@mock.patch("insights.client.auto_config.rhsm.config.get_config_parser")
def test_autoconfigure_network(get_config_parser, _cert_exists, rhel, config_legacy_upload, expected_url):
    rhsm_config = configparser.ConfigParser()
    rhsm_config.read_string(
        "[server]\nhostname=subscription.rhsm.redhat.com\nport=443\n"
        "proxy_scheme=https\nproxy_hostname=proxy.internal\nproxy_port=443\n"
    ...
    client_config = mock.Mock(offline=False, auto_config=True, legacy_upload=config_legacy_upload)
