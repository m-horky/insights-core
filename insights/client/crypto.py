...
class GPGCommand(object):
    ...
    def _cleanup(self):
        ...
        shutdown_process = subprocess.Popen(
            ["/usr/bin/gpgconf", "--homedir", self._home, "--kill", "all"],
            stdout=subprocess.PIPE, stderr=subprocess.PIPE,
            universal_newlines=True,
        )
        _, _ = shutdown_process.communicate()
        if shutdown_process.returncode != 0:
            logger.debug(
                "Could not kill the GPG agent, got return code {rc}.".format(rc=shutdown_process.returncode)
            )
