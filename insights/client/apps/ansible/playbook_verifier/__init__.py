...
class PlaybookVerificationError(Exception):
    ...
    def __init__(self, message="Could not verify the Ansible playbook"):
...
def read_playbook_yaml_from_stdin():
    """Read the Ansible playbook source from stdin.

    :returns: The parsed playbook
    :rtype: dict
    """
    playbook = ""
    for line in sys.stdin:
        playbook += line
    return load_playbook_yaml(playbook)
...
def hash_playbook_snippet(snippet):
    ...
    :returns: The Sha256 hash of the snippet.
    :rtype: bytes
    """
    sha = hashlib.sha256()
    if six.PY2:
...
def exclude_dynamic_elements(snippet):
    ...
    for element in exclusions:
        elements = [string for string in element.split("/") if string != '']
...
def verify_playbook_snippet(snippet):
    ...
    cleaned_snippet = exclude_dynamic_elements(snippet)
    encoded_signature = snippet['vars'][PLAYBOOK_SIGNATURE_LABEL]
...
def verify(playbook):
    ...
    for snippet in playbook:
        ...
        for revoked_item in revocation_list:
            if snippet_hash == bytearray.fromhex(revoked_item['hash']):
